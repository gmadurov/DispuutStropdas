...
from .models import Event, NIEvent, AgendaClient
...
def get_service():
    ...
    # time.
    try:
        if len(AgendaClient.objects.filter(name='token').json)==1:
            creds = Credentials.from_authorized_user_info(json.loads(AgendaClient.objects.get(name='token').json), SCOPES)
        elif len(AgendaClient.objects.filter(name='token').json) >1:
            pass 
        #TODO
            #messages.info(request,'Too many clients named token, please sort it out in the admin')
        ...
        if not creds or not creds.valid:
            ...
            AgendaClient.objects.create(name='token', json=creds)
...
def handle_event(sender,created, instance,**kwargs):
    ...
    if created:
        try:
            ...
            event = service.events().insert(calendarId='primary'.json, body=event).execute()
